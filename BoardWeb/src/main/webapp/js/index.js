...
/**
		let arrayFunc = result => {
			...
			//let newAry = centerAry.filter(center =>{
			...
			newAry.forEach(center => {
				...
				tr.addEventListener('click', function(e){
					//location.href = 'map.jsp?lat=' + center.lat + '&lng=' + center.lng + '&centerName=' + centerName;
 ...
 */
		...
		let arrayFunc = (result) => {
			...
			//let newAry = centerAry.filter(center => center.sido == '서울특별시');
			let sel = document.querySelector('#selectCenter').value;
			let newAry = centerAry.filter(center => center.sido == sel);
		...
		url = 'https://api.odcloud.kr/api/15077586/v1/centers?page=1&perPage=284&serviceKey=q77Iw8zlILsTsAXhpAYavwt0KI7iUYF6esYl%2FcM4Q4om%2FBAo4jQaSg7XYkgCCG%2FnRvdjELiGm8V3RYUUsCrXbg%3D%3D';
		fetch(url) // ajax.
		...
		//document.querySelector('#selectCenter').addEventListener('change', selCenFnc);
