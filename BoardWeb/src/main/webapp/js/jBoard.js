...
function makeRow(reply = {}) {
	...
	*/
	let cloned = $('div.reply>div.content li:first').clone();
	cloned.attr('data-rno', reply.replyNo);
	cloned.css('display', 'block');
	cloned.children('span:eq(0)').text(reply.replyNo);
	cloned.children('span').eq(1).text(reply.replyContent);
	cloned.children('span').eq(2).text(reply.replyer);
	cloned.children('button').on('click', deleteReplyFnc);
